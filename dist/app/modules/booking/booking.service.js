...
const createBooking = async (payload, userId) => {
    ...
    try {
        ...
        const updateBooking = await Booking.findByIdAndUpdate(booking._id, { payment: payment._id }, { new: true, runValidators: true, session })
            .populate("user", "name email phone address")
            .populate("tour", " title costFrom")
            .populate("payment");
        const userAddress = updateBooking?.user?.address || "";
        const userEmail = updateBooking?.user?.email || "";
        const userName = updateBooking?.user?.name || "";
        const userPhone = updateBooking?.user?.phone || "";
        const sslPayload = {
            address: userAddress,
            email: userEmail,
            name: userName,
