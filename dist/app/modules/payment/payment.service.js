...
const successPayment = async (transactionId, gatewayData) => {
    ...
    try {
        ...
        const updatedBooking = await Booking.findByIdAndUpdate(updatedPayment.booking, { status: BOOKING_STATUS.CONFIRM }, { new: true, runValidators: true, session })
            .populate("tour", "title")
            .populate({
            path: "user",
            select: "name +email"
        ...
        const bookingCreatedAt = bookingCreatedAtRaw instanceof Date
            ? bookingCreatedAtRaw
...
const failPayment = async (transactionId, gatewayData) => {
    const session = await Booking.startSession();
    session.startTransaction();
    try {
        const updatePayment = await Payment.findOneAndUpdate({ transactionId }, { status: PAYMENT_STATUS.FAILED, paymentGatewayData: gatewayData }, { new: true, session });
        if (!updatePayment) {
            throw new AppError(401, "Payment not found");
        }
        await Booking.findByIdAndUpdate(updatePayment?.booking, { status: BOOKING_STATUS.FAILED }, { returnDocument: "after", session });
...
const cancelPayment = async (transactionId, gatewayData) => {
    const session = await Booking.startSession();
    session.startTransaction();
    try {
        const updatePayment = await Payment.findOneAndUpdate({ transactionId }, { status: PAYMENT_STATUS.CANCELLED, paymentGatewayData: gatewayData }, { new: true, session });
        if (!updatePayment) {
            throw new AppError(401, "Payment not found");
        }
        await Booking.findByIdAndUpdate(updatePayment?.booking, { status: BOOKING_STATUS.CANCEL }, { returnDocument: "after", session });
        await session.commitTransaction();
        session.endSession();
        return {
            success: true,
